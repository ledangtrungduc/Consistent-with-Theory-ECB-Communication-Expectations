...
* `Code/`: Contains the refactored, modularized scripts optimized for reproducibility:
    * `00_master.do`: The master script that coordinates the entire data pipeline and analysis.
    * `01_data_cleaning.do`: Processes raw CES microdata, merges macroeconomic indicators, and constructs theory-consistent measures.
...
* `results/`: Stores the output files including regression tables (`.rtf`) and event-study plots (`.png`).
